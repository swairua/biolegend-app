          ...
          <table class="items-table">
            ...
            <tbody>
              ...
              ` : data.type === 'remittance' ? `
                  ...
                  <td style="font-weight: bold; color: ${(Number((item as any).quantity_delivered ?? (item as any).quantity ?? 0) >= Number((item as any).quantity_ordered ?? 0)) ? '#10B981' : '#F59E0B'};">${formatQuantity((item as any).quantity_delivered ?? (item as any).quantity ?? 0)}</td>
                  <td style="font-size: 10px;">
                    ${(Number((item as any).quantity_delivered ?? (item as any).quantity ?? 0) >= Number((item as any).quantity_ordered ?? 0))
...
export const downloadCreditNotePDF = async (creditNote: any, company?: CompanyDetails) => {
  const documentData: DocumentData = {
    ...
    items: creditNote.credit_note_items?.map((item: any, index: number) => {
      const quantity = Number(item.quantity || 0);
      const unitPrice = Number(item.unit_price || 0);
      const taxAmount = Number(item.tax_amount || 0);
      const discountAmount = Number(item.discount_amount || 0);
